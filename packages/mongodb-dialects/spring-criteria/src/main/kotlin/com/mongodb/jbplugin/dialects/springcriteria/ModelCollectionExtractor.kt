...
object ModelCollectionExtractor {
    fun fromPsiClass(clazz: PsiClass): String? {
        ...
        annotation?.let {
val collectionName = annotation.findAttributeValue("value")?.tryToResolveAsConstantString().takeIf {
 !it
.isNullOrBlank()
}
 ?: annotation.findAttributeValue("collection")?.tryToResolveAsConstantString().takeIf { !it.isNullOrBlank() }
collectionName?.let {
return collectionName
}
/* it's the class name but with the first letter lower cased:
https://docs.spring.io/spring-data/mongodb/docs/3.0.1.RELEASE/reference/html/#mongo-template.save-insert.collection
*/
